...
@section('subheading', 'Perbarui informasi produk')
        ...
        <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <textarea name="description" rows="3" required
                    class="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 resize-none">{{ old('description', $product->description) }}</textarea>
            ...
            <div class="mb-4">
                ...
                <div class="mb-3 flex items-center gap-3">
                    <img src="{{ Storage::url($product->image) }}" class="w-20 h-20 object-cover rounded-xl" alt="Foto saat ini">
                    <div class="text-xs text-gray-400">Foto saat ini. Upload baru untuk mengganti.</div>
                </div>
                @endif
                <div class="border-2 border-dashed border-gray-200 rounded-xl p-5 text-center hover:border-green-400 transition-colors cursor-pointer relative">
                    <input type="file" name="image" accept="image/*" class="absolute inset-0 opacity-0 cursor-pointer"
                        onchange="previewImage(this)">
                    <div id="dropzoneContent">
                        <i data-lucide="upload-cloud" class="w-7 h-7 text-gray-300 mx-auto mb-1.5"></i>
                        <p class="text-sm text-gray-400">Klik untuk upload foto baru</p>
                        <p class="text-xs text-gray-300 mt-0.5">JPG, PNG, WebP maks. 2MB</p>
                    </div>
                    <img id="imagePreview" class="hidden mx-auto max-h-36 rounded-lg" alt="Preview">
                </div>
